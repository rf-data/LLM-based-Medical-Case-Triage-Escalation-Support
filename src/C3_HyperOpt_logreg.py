...
import utils.hyperopt_helper as hyper
...
from D4_hyper_opt import run_HyperOpt


# --- DEFINE CONFIGURATION FILE(S) ----------------------
from configuration.C4_logreg_HyperOpt import config, num_feats, cat_feats, param_grid
# from configuration.C4_2_logreg_HyperOpt_kfoldshuffle import config_2
# ----------------------------------------------------
...
def logreg_HyperOpt(logging=False):
    ...
    split_hyperopt=session.parameters.get("split_mode_hyperopt", "tba")
    split_vali=session.parameters.get("split_mode_validation", "tba")
    ...
    param_combinations = list(
                            itertools.product(
                                param_grid["clf__C"],
                                param_grid["clf__l1_ratio"],
                                param_grid["clf__class_weight"],
                                )
                            )

    event_logger.info(
        "Hyperparameter combinations: %d", len(param_combinations)
    )
    ...
    # start hyperparameter optimisation (LogReg)
    now = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    event_logger.info("Start LogReg Hyperparameter optimisation: %s", now)
    session.now = now

    # -----------------------------
    # Grid loop + 
    # -----------------------------
    results, thresh_df_hyper, skipped_splits = run_HyperOpt(pipe, 
                                                            param_combinations, 
                                                            lambda: split.group_split(X, 
                                                                                      y, 
                                                                                      split_hyperopt), 
                                                            metric="f2")

    # sort result_df + extract topk_df
    sort_cols = ["q25_f2", "median_f2", "mean_f2"]
    extract_cols = ["C", "l1_ratio", 
                    "class_weight", "median_f2", 
                    "mean_f2", "q25_f2"]
    results_df = pd.DataFrame(results)

    df_sort, top3_df = fh.sort_extract_df(
                                    results_df, 
                                    sort_cols, 
                                    extract_cols, 
                                    k=3
    ...
    sort_path = ph.create_save_path(
            "hyperopt",
            "result_hyper",
            ".csv",
        )
    df_sort.to_csv(sort_path, index=False)
    exp_logger.log_artifact(sort_path)

    event_logger.info("Hyperparameter optimization finished")

    # -----------------------------------------------
    # Threshold sweep analysis + best_t validation 
    # -----------------------------------------------
    event_logger.info("Starting threshold sweep analysis with top3 settings")
    
    result_df, thresh_df = hyper.validate_topk_model(
                                                top3_df,
                                                pipe,
                                                lambda: split.group_split(X, 
                                                                          y, 
    ...
    result_path = ph.create_save_path(f"hyperopt", "results_vali", ".csv")
    result_df.to_csv(result_path)
    exp_logger.log_artifact(result_path)

    # create + save thresh_df
    thresh_path = ph.create_save_path("hyperopt", "thresh_vali", ".csv")
    thresh_hyper_path = ph.create_save_path("hyperopt", "_thresh_hyper", ".csv")
    thresh_df.to_csv(thresh_path)
    thresh_df_hyper.to_csv(thresh_hyper_path)

    # add a comment
    if skipped_splits:
        notes = f"""
The following splits were skipped due to empty slices:
{skipped_splits}
        """
        exp_logger.log_text(
        "analysis/01_skipped_splits.md",
        notes
            )
# ### Initial Observation
